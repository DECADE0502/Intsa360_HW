...
_UPDATE_MESSAGE_TRANSLATIONS = {
    ...
    "Existing verified runtime will be reused.": "已存在内容一致的完整版本，将直接复用。",
    "Moving the verified runtime into the installation atomically.": "正在将已验证版本原子移动到安装目录。",
    "Candidate runtime installed by atomic directory move.": "新版本已通过原子目录移动完成安装。",
    "Staging and installation are on different volumes; copying the verified runtime.": "暂存区与安装目录不在同一磁盘，正在复制并复核完整版本。",
...
def _update_status_payload(raw: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "detail_bytes_current": _number(raw.get("detail_bytes_current"), 0),
        "detail_bytes_total": _number(raw.get("detail_bytes_total"), 0),
