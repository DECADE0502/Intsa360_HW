...
def _manifest_confirms_current_release(root: Path, remote_version: str, remote_revision: str) -> bool:
    """True when install_manifest.json records exactly the remote release.

    The manifest is generated by build_release.ps1 and synced with every
    release-zip payload; unlike REVISION it is never rewritten by updaters,
    so it is the reliable record of which authored release is installed.
    """
    ...
    return bool(revision) and version == remote_version and revision == remote_revision


def _self_heal_local_revision(root: Path, authored_revision: str, current_revision: str) -> str:
    try:
        (root / "REVISION").write_text(authored_revision + "\n", encoding="utf-8")
...
def check_update(root: Path) -> dict[str, object]:
    ...
    if remote_status in _REMOTE_VERSION_OK_STATUSES and remote_version:
        ...
        if remote_cmp > 0:
            ...
            #
            # Exception: updaters up to 0.2.26 overwrote REVISION with the
            # resolved git head (the release commit) instead of keeping the
            # authored value (its parent), so REVISION never equals the remote
            # file again after a successful update. install_manifest.json is
            # untouched by those updaters and still records the authored
            # revision - when it matches the remote value we already run this
            # release, so self-heal the stamp instead of advertising a phantom
            # hotfix.
            if _manifest_confirms_current_release(root, remote_version, remote_revision):
                local_revision = _self_heal_local_revision(root, remote_revision, local_revision)
            else:
                has_update = True
